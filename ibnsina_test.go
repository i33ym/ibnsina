...
func TestMatch(t *testing.T) {
	...
	for _, test := range tests {
		...
		handler := func(context context.Context, response http.ResponseWriter, request *http.Request) {
			ctx = request.Context()
		}

		router.Handle(test.RoutePattern, handler, test.RouteMethods...)

		request, err := http.NewRequest(test.RequestMethod, test.RequestPath, nil)
		if err != nil {
...
// 		fn := func(ctx context.Context, response http.ResponseWriter, request *http.Request) {
// 			used = used + "1"
...
// 	handler := func(ctx context.Context, response http.ResponseWriter, request *http.Request) {}
...
// 	router.Handle("/", handler, "GET")

// 	router.Group(func(router *Router) {
// 		router.Use(mw3, mw4)
// 		router.Handle("/foo", handler, "GET")

// 		router.Group(func(router *Router) {
// 			router.Use(mw5)
// 			router.Handle("/nested/foo", handler, "GET")
// 		})
// 	})

// 	router.Group(func(router *Router) {
// 		router.Use(mw6)
// 		router.Handle("/bar", handler, "GET")
// 	})

// 	router.Handle("/baz", handler, "GET")
...
// 			t.Errorf("%s %s: expected status %d but was %d", test.RequestMethod, test.RequestPath, test.ExpectedStatus, response.StatusCode)
// 		}
...
func TestCustomHandlers(t *testing.T) {
	...
	router.NotFound = func(ctx context.Context, response http.ResponseWriter, request *http.Request) {
		response.Write([]byte("custom not found handler"))
	}

	router.MethodNotAllowed = func(ctx context.Context, response http.ResponseWriter, request *http.Request) {
		response.Write([]byte("custom method not allowed handler"))
	}

	router.Options = func(ctx context.Context, response http.ResponseWriter, request *http.Request) {
		response.Write([]byte("custom options handler"))
	}
